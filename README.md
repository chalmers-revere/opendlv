...
* Command to run with Docker: `docker run --init --rm --net=host chalmersrevere/opendlv-device-lidar-rplidar-multi:v0.0.1 --device=/dev/ttyUSB0 --cid=111 --verbose`
* Section for `docker-compose.yml`:
```yml
version: '2' # Must be present exactly once at the beginning of the docker-compose.yml file
services:    # Must be present exactly once at the beginning of the docker-compose.yml file
    dev-lidar-rplidar:
        ...
        image: chalmersrevere/opendlv-device-lidar-rplidar-multi:v0.0.1
        restart: on-failure
        network_mode: "host"
        devices:
