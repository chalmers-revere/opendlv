...
* Command to run with Docker: `docker run --rm -ti --init --ipc=host -e DISPLAY=$DISPLAY --device /dev/video0 -v /tmp:/tmp chalmersrevere/opendlv-device-camera-rpi-armhf:v0.0.2 --width=640 --height=480 --freq=20`
* Section for `docker-compose.yml`:
```yml
version: '2' # Must be present exactly once at the beginning of the docker-compose.yml file
services:    # Must be present exactly once at the beginning of the docker-compose.yml file
    dev-camera-rpi:
        image: chalmersrevere/opendlv-device-camera-rpi-armhf:v0.0.2
        restart: on-failure
        ipc: "host"
        volumes:
        - /tmp:/tmp
