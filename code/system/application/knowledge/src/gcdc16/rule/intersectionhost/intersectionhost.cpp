...
*/
/*
double IntersectionHost::getDistances(double hostVelocity)
{
...
bool IntersectionHost::euclideanDistance(double measuredDistance)
{
...
void IntersectionHost::receivedContainerMergeScenario(odcore::data::Container &a_container)
{
  ...
  if (a_container.getDataType() == opendlv::perception::Environment::ID()) {
    ...
    if (objects.size() < 2) {
      std::cout << "ERROR: intersectionhost.cpp only received info about " << objects.size() << " objects..." << std::endl;
      return;
    }
...
void IntersectionHost::bodyMergeScenario()
{
  odcore::data::TimeStamp timestamp;
...
void IntersectionHost::receivedContainerIntersectionScenario(odcore::data::Container &a_container)
{
...
void IntersectionHost::bodyIntersectionScenario()
{
  /*
...
void IntersectionHost::setUp()
{
  ...
  m_currentLane = kv.getValue<std::string>("knowledge-gcdc16-intersectionhost.initial_lane");
  m_isTail = kv.getValue<std::string>("knowledge-gcdc16-intersectionhost.is_tail");
  m_platoonId = kv.getValue<std::string>("knowledge-gcdc16-intersectionhost.platoon_id");
  m_scenarioType = kv.getValue<std::string>("knowledge-gcdc16-intersectionhost.scenario");
