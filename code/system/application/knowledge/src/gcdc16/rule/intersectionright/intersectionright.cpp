...
*/
/*
double IntersectionRight::getDistances(double hostVelocity)
{
...
bool IntersectionRight::euclideanDistance(double measuredDistance)
{
...
void IntersectionRight::receivedContainerMergeScenario(odcore::data::Container &a_container)
{
  ...
  if (a_container.getDataType() == opendlv::perception::Environment::ID()) {
    ...
    if (objects.size() < 2) {
      std::cout << "ERROR: intersectionright.cpp only received info about " << objects.size() << " objects..." << std::endl;
      return;
    }
...
void IntersectionRight::bodyMergeScenario()
{
  odcore::data::TimeStamp timestamp;
...
void IntersectionRight::receivedContainerIntersectionScenario(odcore::data::Container &a_container)
{
...
void IntersectionRight::bodyIntersectionScenario()
{
  /*
...
void IntersectionRight::setUp()
{
  ...
  m_currentLane = kv.getValue<std::string>("knowledge-gcdc16-intersectionright.initial_lane");
  m_isTail = kv.getValue<std::string>("knowledge-gcdc16-intersectionright.is_tail");
  m_platoonId = kv.getValue<std::string>("knowledge-gcdc16-intersectionright.platoon_id");
  m_scenarioType = kv.getValue<std::string>("knowledge-gcdc16-intersectionright.scenario");
