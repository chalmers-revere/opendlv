...
static float IC_Angle(const cv::Mat& image, cv::Point2f pt,  const std::vector<int> & u_max) //Static??
{
    int m_01 = 0, m_10 = 0;
...
static void computeOrbDescriptor(const cv::KeyPoint& kpt, const cv::Mat& img, const cv::Point* pattern, uchar* desc)//Static??
{
...
static void computeDescriptors(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors, const std::vector<cv::Point>& pattern)//static??
{
...
OrbExtractor::OrbExtractor(int nFeatures, float scaleFactor, int nLevels, int initialFastTh, int minFastTh):
    ...
    const cv::Point* pattern0 = (cv::Point*)bit_pattern_31_; //Can we use shared_ptr here, do we need to?
    std::copy(pattern0, pattern0 + npoints, std::back_inserter(m_vPattern));
...
void OrbExtractor::ExtractFeatures(cv::InputArray inputImage,std::vector<cv::KeyPoint>& a_keypoints,cv::OutputArray a_descriptors){
  ...
  if( nKeypoints == 0 )
    {
    a_descriptors.release();
    }
  else
  {
    a_descriptors.create(nKeypoints, 32, CV_8U);
    descriptors = a_descriptors.getMat();
  }
    a_keypoints.clear();
  a_keypoints.reserve(nKeypoints);
  ...
  for (int level = 0; level < m_nLevels; ++level)
    ...
    cv::Mat workingMat = m_vImagePyramid[level].clone();
    ...
    computeDescriptors(workingMat, keypoints, desc, m_vPattern);
...
void OrbExtractor::ComputePyramid(cv::Mat image)
{
    for (int level = 0; level < m_nLevels; ++level)
    {
        float scale = m_vInvScaleFactor[level];
        ...
        if( level != 0 )
        {
            resize(m_vImagePyramid[level-1], m_vImagePyramid[level], sz, 0, 0, cv::INTER_LINEAR);

            copyMakeBorder(m_vImagePyramid[level], temp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD,
...
void OrbExtractor::ComputeKeyPointsOctTree(std::vector<std::vector<cv::KeyPoint> >& allKeypoints)
{
  ...
  for (int level = 0; level < m_nLevels; ++level)
  {
    ...
    const int maxBorderX = m_vImagePyramid[level].cols-EDGE_THRESHOLD+3;
    const int maxBorderY = m_vImagePyramid[level].rows-EDGE_THRESHOLD+3;

    std::vector<cv::KeyPoint> vToDistributeKeys;
    vToDistributeKeys.reserve(m_nFeatures*10);
        ...
        for(int j=0; j<nCols; j++)
          {
            ...
            if(maxX>maxBorderX)
             ...
             cv::FAST(m_vImagePyramid[level].rowRange(iniY,maxY).colRange(iniX,maxX),
                     vKeysCell,m_initialFastTh,true);

             if(vKeysCell.empty())
             {
                cv::FAST(m_vImagePyramid[level].rowRange(iniY,maxY).colRange(iniX,maxX),
                         vKeysCell,m_minFastTh,true);
             }
                                       ...
                                       const int &maxX, const int &minY, const int &maxY, const int &N)
{
    ...
    std::list<ExtractorNode>::iterator nodeIterator = lNodes.begin();

    while(nodeIterator!=lNodes.end())
    {
        if(nodeIterator->vKeys.size()==1)
        {
            nodeIterator->bNoMore=true;
            nodeIterator++;
        }
        else if(nodeIterator->vKeys.empty())
    ...
    while(!bFinish)
    {
        ...
        while(nodeIterator!=lNodes.end())
        {
            ...
            else
            {
                ...
                nodeIterator->DivideNode(n1,n2,n3,n4);
    ...
    vResultKeys.reserve(m_nFeatures);
